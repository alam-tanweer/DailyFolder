...
public class Program {
    ...
    public void partA(List<Student> students) {
        students.stream().filter(s -> s.getGrade() > 85).forEach(student -> System.out.println(student.getName() + " - " + student.getGrade()));   
    }

    //Find Adult Student
    public void partB(List<Student> students) {
        students.stream().filter(s -> s.getAge() >= 18).forEach(student -> System.out.println(student.getName() + " - " + student.getAge()));   
    }
