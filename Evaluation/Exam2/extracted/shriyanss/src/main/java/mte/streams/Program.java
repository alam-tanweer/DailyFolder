...
public class Program {
    ...
    public void partA(List<Student> students) {
        students.stream().filter(s -> s.getGrade() > 85).forEach(s -> System.out.println(s.getName() + " - " + s.getGrade()));
        // students.forEach(s -> System.out.println(s.getName() + " - " + s.getGrade()));
    }

    //Find Adult Student
    public void partB(List<Student> students) {
        // Arrays.stream(students).filter(s -> s.getAge() > 18).forEach(s -> System.out.println(s.getName() + " - " + s.getAge()));
        students.stream().filter(s -> s.getAge() >= 18).forEach(s -> System.out.println(s.getName() + " - " + s.getAge()));
