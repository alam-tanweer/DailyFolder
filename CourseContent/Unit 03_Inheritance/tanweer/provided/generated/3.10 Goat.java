package unit03.gvt;

/**
 * Parent class that encapsulates the common state and behavior
 * shared by all goats.
 */
public abstract class Goat {

    private String name;
    private int maximumHP;
    private int currentHP;

    /**
     * Constructs a Goat with the given name and maximum HP.
     * Current HP starts at maximum.
     *
     * @param name the goat's name
     * @param maximumHP the goat's maximum hit points
     */
    public Goat(String name, int maximumHP) {
        this.name = name;
        this.maximumHP = maximumHP;
        this.currentHP = maximumHP;
    }

    // Accessors (getters)
    public String getName() {
        return name;
    }

    public int getMaximumHP() {
        return maximumHP;
    }

    public int getCurrentHP() {
        return currentHP;
    }

    /**
     * Heals this goat by the specified amount, not exceeding max HP.
     *
     * @param amount amount of HP to restore
     */
    public void heal(int amount) {
        currentHP += amount;
        if (currentHP > maximumHP) {
            currentHP = maximumHP;
        }
    }

    /**
     * @return true if current HP is greater than 0
     */
    public boolean isConscious() {
        return currentHP > 0;
    }

    /**
     * All goats need an attack, but there is no common implementation.
     */
    public abstract Attack attack();

    /**
     * All goats need to take damage, but there is no common implementation.
     *
     * @param attack the incoming attack
     */
    public abstract void takeDamage(Attack attack);

    /**
     * Allows subclasses to reduce current HP safely (not below 0).
     *
     * @param amount damage amount to subtract
     */
    protected void reduceHP(int amount) {
        currentHP -= amount;
        if (currentHP < 0) {
            currentHP = 0;
        }
    }

    @Override
    public String toString() {
        return "A goat named " + name + " with " + currentHP + "/" + maximumHP + " hit points!";
    }
}
